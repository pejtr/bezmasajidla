...
import { getOmniForgeConfig } from "./_core/omniforge-client";
...
describe("OMNIFORGE Contract Gate — Database Migration Proof", () => {
  it("should verify physical DB schema columns, indexes, and tables in READ-ONLY mode", async () => {
    ...
    console.log(
      "[PROD DB Migration Proof (Read-Only)]",
...
describe("OMNIFORGE Contract Gate — Atomic Concurrency & Lease Lock", () => {
  it("should prevent dual parallel processing on simultaneous webhook delivery", async () => {
    ...
    const resC = await claimAndCheckWebhookEvent({
      eventId: concurrentEventId,
    });
...
describe("OMNIFORGE Contract Gate — End-to-End Live Connectivity", () => {
  it("should verify OMNIFORGE API reachability or report NOT DEPLOYED status", async () => {
    ...
    const isConfigured = Boolean(config.apiUrl && config.serviceToken);

    if (!isConfigured) {
      console.warn(
        `[OMNIFORGE Gate A] OMNIFORGE_API_URL / OMNIFORGE_SERVICE_TOKEN are not configured. Result: NOT RUN`
      );
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.warn(
          `[OMNIFORGE Gate A] API ${config.apiUrl} returned ${response.status} — NOT DEPLOYED`
        );
      }
    } catch (err: any) {
      console.warn(
        `[OMNIFORGE Gate A] Network connection to ${config.apiUrl} failed: ${err.message} — NOT DEPLOYED`
      );
    }
  });
});
